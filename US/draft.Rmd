...
Results from a TVAR model estimated with a full set of controls indicates that 
...
```{=latex}
\caption{Regime specific impulse responses, updated specification. \label{fig:updatedirfs}}
\bigskip
\footnotesize
\textit{Notes}: In the updated specification, interest rates are measured by the FFR, the threshold value is set to the median of the EPU index, lagged values of the EPU and the St. Louis FED Financial Stress Indicator are added as controls.
\end{figure}
```

These results indicate that some of the findings from both @aastveit2017economic, @aquino2022does and @horvathTBAuncertain can be true at the same time. In high uncertainty times, the impact of the monetary shock is considerably elevated on impact, and the first few quarters after the shock. However, dissipates much more rapidly from the variables, even reverting from disinflation to inflation around 12-16 quarters after the shock. The larger impact around the time of the shock supports the idea, that inflates the variance of expectations, and thereby increasing the impact of shocks.
...
In @baker2016measuring, additional to the EPU index, several categorical indices are constructed[^Please refer to Figure \ref{fig:epucat}.]. To investigate how the underlying cause behind uncertainty interacts with the transmission mechanism, I replicate the estimation with each of these categorical indices. For easier comparability, I calculate sacrifice ratios - foregone output as a fraction of deterred inflation - from the 20-quarter accumulated impulse responses.

Replicating the simulations with each categorical index could provide insight into how such narratives can impact the transmission mechanism. Notably, uncertainty specifically related to monetary policy should be the most insightful. The sacrifice ratios can be seen in Figure \ref{fig:narrative} below.
...
\bigskip
\footnotesize
\textit{Notes}: The Full control TVAR model is used for estimating the data behind the sacrifice ratios.  
\end{figure}
```

\textcolor{red}{CALCULATE OVER DIFFERENT HORIZONS INSTEAD OF THE TWO MU-s YOU DUMMY! SHORT TERM GAINS OVER LONG TERM LOSSES}
